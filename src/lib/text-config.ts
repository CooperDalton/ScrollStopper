...
export const FONT_SIZES = [12, 14, 16, 20, 24, 32, 40, 48, 56, 64] as const
...
export const STROKE_WIDTHS = [0.75, 1.15, 1.5,2.25,3,3.75,4.5,5.25,6,6.75] as const

// Maximum characters per line for each font size
//export const MAX_CHARS_PER_LINE = [54, 40, 34, 30, 26, 22, 18, 14, 10] as const
export const MAX_CHARS_PER_LINE = [40, 36, 32, 26, 20, 18, 16, 14, 10, 6] as const
...
export const getStrokeWidthForFontSize = (fontSize: number): number => {
  ...
  // Base the calculation on the ratio of font sizes and corresponding stroke widths
  if (fontSize < FONT_SIZES[0]) {
    // For very small fonts, use the smallest stroke width
    return STROKE_WIDTHS[0]
  }

  if (fontSize > FONT_SIZES[FONT_SIZES.length - 1]) {
    // For large fonts, scale proportionally from the largest predefined size
